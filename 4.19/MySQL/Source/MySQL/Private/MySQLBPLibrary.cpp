...
wstring_convert<codecvt_utf8_utf16<wchar_t>> converter;
		...
		*/
        ...
        static FString GameDllFilePath =  FPaths::Combine(*FPaths::ProjectPluginsDir(), TEXT("MySQL/SQLLib"), PlatformString, TEXT("SQL.dll"));
		...
		*/
		static FString MarketplaceDllFilePath = FPaths::Combine(*FPaths::EnginePluginsDir(), TEXT("Marketplace/MySQL/SQLLib") , PlatformString , TEXT("SQL.dll"));
		...
		*/
		static FString EngineDllFilePath = FPaths::Combine(*FPaths::EnginePluginsDir(), TEXT("Marketplace/MySQL/SQLLib"), PlatformString, TEXT("SQL.dll"));
...
static FString GetFilePath()
{
	...
	FString DllPath = FPaths::Combine(*Pluginpath, TEXT("SQLLib"), PlatformString, TEXT("SQL.dll"));
...
string EncodeString(FString inputstring)
{
	wstring wStr = *inputstring;
	string narrow = converter.to_bytes(wStr);
	return narrow;
}

FString DecodeString(string encodedstring)
{
	wstring wide = converter.from_bytes(encodedstring);
	const wchar_t *str = wide.c_str();
	FString result = FString(str);
	return result;
}

FString DecodeFString(FString encodedstring)
{
	wstring wStr = *encodedstring;
	string s(wStr.begin(), wStr.end());

	const char *str = s.c_str();

	FString result = DecodeString(s);
	string s1 = TCHAR_TO_UTF8(*result);
...
{
	string query = EncodeString(Query);
	char* q = _strdup(query.c_str());
